...
class DbInitializer:
    def __init__(self):
        connector = mysql.connector.connect(
            host="localhost", user=user_login, password=user_password
        )
        ...
        self.connector = mysql.connector.connect(
            host="localhost", user="root", password=user_password, database="theatre"
        )
...
class DumpGenerator:
    ...
    def create_dump(connector):
        ...
        try:
            ...
            if log:
                print("\t-Actor-genre-link dump created successfully.")
        except mysql.connector.Error as error:
            if log:
...
def show_all():
    message = db_manager.print_select_all("actor")
    message += db_manager.print_select_all("event")
    message += db_manager.print_select_all("impresario")
    message += db_manager.print_select_all("genre")
    message += db_manager.print_select_all("building")
    messagebox.showinfo("show_all", message)


def show_actors():
    messagebox.showinfo("show_actors", db_manager.print_select_all("actor"))


def show_events():
    messagebox.showinfo("show_events", db_manager.print_select_all("event"))


def show_impresarios():
    messagebox.showinfo("show_impresarios", db_manager.print_select_all("impresario"))


def show_genres():
    messagebox.showinfo("show_genres", db_manager.print_select_all("genre"))


def show_buildings():
    messagebox.showinfo("show_buildings", db_manager.print_select_all("building"))
...
def handle_command():
    ...
    if command == "help":
        ...
        show_all()
    elif command == "show_actors":
        show_actors()
    elif command == "show_events":
        show_events()
    elif command == "show_impresarios":
        show_impresarios()
    elif command == "show_genres":
        show_genres()
    elif command == "show_buildings":
        show_buildings()
    elif command == "exit":
        root.destroy()
    # Добавьте остальные ветви для остальных команд
    else:
        messagebox.showerror("Error", "Invalid command. Type 'help' for help.")
...
button = tk.Button(root, text="Submit", command=handle_command)
button.pack()
